...
public class Pensionar extends Utilizator {
    ...
    void afisareCereriAsteptare(BufferedWriter bw) throws IOException, ParseException {
        ...
        for (int i = 0; i < this.cerereAsteptare.size(); i++)
            bw.write(cerereAsteptare.get(i).getData() + " - Subsemnatul "
                    + cerereAsteptare.get(i).getNume() + ", va rog sa-mi aprobati urmatoarea solicitare: "
                    + cerereAsteptare.get(i).getCerere() + "\n");
    }
    ...
    void afisareCereriFinalizate(BufferedWriter bw) throws IOException, ParseException {
        ...
        for (int i = 0; i < this.cerereFinalizata.size(); i++)
            bw.write(cerereFinalizata.get(i).getData() + " - Subsemnatul "
                    + cerereFinalizata.get(i).getNume() + ", va rog sa-mi aprobati urmatoarea solicitare: "
                    + cerereFinalizata.get(i).getCerere() + "\n");
    }
    ...
    void adaugaCerere(BufferedWriter bw, String[] actiune) {
        ...
        if (!cerere.equals("inlocuire buletin")
                ...
                && !cerere.equals("inregistrare cupoane de pensie"))
            try {
                actiuneIncorecta(bw, "pensionar", cerere);
            ...
            cerereAsteptare.add(new CererePensionar(nume, cerere, data, prioritate));
        }
    }
}